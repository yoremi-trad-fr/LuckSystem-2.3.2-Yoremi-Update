...
func (cz *Cz3Image) Import(r io.Reader, fillSize bool) error {
	...
	if int(cz.Width) != pic.Rect.Dx() || int(cz.Heigth) != pic.Rect.Dy() {
		return fmt.Errorf("CZ3 import size mismatch: PNG is %dx%d, expected %dx%d",
			pic.Rect.Dx(), pic.Rect.Dy(), cz.Width, cz.Heigth)
	}
	...
	if len(data) == 0 {
		return fmt.Errorf("CZ3 import produced empty pixel data")
	}
